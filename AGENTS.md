...
- `USE_VARIABLE_CHECK_INTERVALS`: Respect per-site `check_interval`; the scheduler uses a short idle poll and maintained `jetmon_endpoint_runtime.next_check_at` timestamps control which endpoints are ready in legacy round-scheduler mode
...
**Check Process:**
- Default timeout: `NET_COMMS_TIMEOUT` seconds (configurable per endpoint via `jetmon_endpoint_check_config.timeout_seconds`)
...
- Per-site custom headers merged from `jetmon_endpoint_check_config.custom_headers`
...
Shadow-v2-state migration keeps incidents authoritative in `jetmon_events` + `jetmon_event_transitions` while `jetpack_monitor_sites` remains the v1-owned site identity/cadence/projection table. V2-only check config lives in `jetmon_endpoint_check_config`, with `jetmon_site_check_config` as a blog-level fallback. When `LEGACY_STATUS_PROJECTION_ENABLE` is true, the `jetpack_monitor_sites.site_status` / `last_status_change` projection is updated in the same transaction as every event mutation (no drift). v1 mapping: open Seems Down → `site_status = SITE_DOWN (0)`; promoted to Down → `site_status = SITE_CONFIRMED_DOWN (2)`; closed → `site_status = SITE_RUNNING (1)`. After legacy readers move to the v2 API/event tables, this projection can be disabled.

**Alert Deduplication:**
After an alert fires, subsequent alerts for the same endpoint are suppressed for the global `ALERT_COOLDOWN_MINUTES` value or `jetmon_endpoint_check_config.alert_cooldown_minutes`. Suppression is recorded in the audit log.
...
v2 keeps v2-only endpoint config and runtime state out of that legacy table:
rich probe config lives in `jetmon_endpoint_check_config`, and freshness / SSL
observation state lives in `jetmon_endpoint_runtime`. During rollout, v2 writes
...
| `jetmon_endpoint_check_config` | V2-only per-endpoint check policy/config: HEAD/GET mode, detection profile, keywords, maintenance windows, headers, timeout, redirect policy, cooldown |
| `jetmon_endpoint_runtime` | V2-only per-endpoint runtime freshness and observation projection: last checked, next check, last alert, SSL expiry |
