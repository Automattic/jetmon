...
var apiCommandCatalog = []apiCommandInfo{
	...
	{Command: "rollout smoke", Description: "run read-only sampled HEAD/GET rollout probes", Example: "jetmon2 api rollout smoke --bucket-min 0 --bucket-max 99 --mode head-legacy --sample-size 100 --read-only --canary-file rollout-canaries.json --allow-remote"},
