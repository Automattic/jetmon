...
configure_runtime_config() {
	...
	case "$mode" in
		always)
			...
			echo "config: hostname=${VERIFLIER_HOSTNAME:-${JETMON_HOSTNAME:-runtime-hostname}} statsd=${STATSD_ADDR:-disabled} statsd_host_path=${VERIFLIER_STATSD_HOST_PATH:-${STATSD_HOST_PATH:-runtime-hostname}} vantage=${VERIFLIER_VANTAGE_ID:-local-veriflier} legacy_http=${VERIFLIER_ENABLE_LEGACY_HTTP:-false} target_safety=${VERIFLIER_CHECK_TARGET_SAFETY_MODE:-${CHECK_TARGET_SAFETY_MODE:-public_only}}"
			;;
		missing)
			target="$(config_render_target)"
			export VERIFLIER_CONFIG="$target"
