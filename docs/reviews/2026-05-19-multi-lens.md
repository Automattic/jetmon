...
### Bugs / races
- ~~**Dual-mutex on veriflier cooldowns** — `internal/orchestrator/orchestrator.go:272` map is protected by `veriflierCooldownMu` (lines 3388, 3424, 3444) but read elsewhere under `veriflierMu` (line 3236+). Unify under one lock or replace with `sync.Map`.~~ **Not reproduced (verified 2026-05-20):** the cited line numbers no longer match — the code was refactored since this review snapshot. In current `v2`, `veriflierCooldowns` is accessed in exactly four methods (`availableVeriflierClients`, `markVeriflierOperationalFailure`, `markVeriflierHealthy`, plus the disjoint `veriflierSnapshot`), and **every** access is under `veriflierCooldownMu`; `veriflierMu` guards only the client list/`veriflierAddrs`, and the two locks are never held simultaneously or nested. No read of the cooldown map occurs under `veriflierMu` or unlocked. Confirmed by static read and by a new concurrent regression test (`TestVeriflierCooldownConcurrentAccessRace`), which passes 20× under `-race`; a negative control (lock removed) trips the detector, proving the test has teeth.
