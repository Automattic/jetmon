...
func TestBuildPayload(t *testing.T) {
	...
	payload, err := buildPayload("alert.opened", 10, 20, 30, sql.NullInt64{Int64: 40, Valid: true}, "opened", "Seems Down", 1, 4, occurredAt)
	if err != nil {
...
func TestProgressLoadSave(t *testing.T) {
	...
	mock.ExpectQuery("SELECT last_transition_id FROM jetpack_monitor_alert_dispatch_progress").
		WithArgs("host-a").
		WillReturnError(sql.ErrNoRows)
	mock.ExpectExec("INSERT INTO jetpack_monitor_alert_dispatch_progress").
