...
const sitesListSQL = ` SELECT s.blog_id, s.blog_id AS public_id, s.monitor_url, s.monitor_active, s.bucket_no, s.check_interval, s.site_status, s.last_checked_at, s.last_status_change, s.ssl_expiry_date, s.check_keyword, s.forbidden_keyword, s.forbidden_keywords, s.redirect_policy, c.request_method, c.detection_profile, s.maintenance_start, s.maintenance_end, s.alert_cooldown_minutes FROM jetpack_monitor_sites s LEFT JOIN jetmon_site_check_config c ON c.blog_id = s.blog_id WHERE s.blog_id > ? ORDER BY s.blog_id ASC LIMIT ?`

const sitesListForTenantSQL = ` SELECT s.blog_id, s.blog_id AS public_id, s.monitor_url, s.monitor_active, s.bucket_no, s.check_interval, s.site_status, s.last_checked_at, s.last_status_change, s.ssl_expiry_date, s.check_keyword, s.forbidden_keyword, s.forbidden_keywords, s.redirect_policy, c.request_method, c.detection_profile, s.maintenance_start, s.maintenance_end, s.alert_cooldown_minutes FROM jetpack_monitor_sites s LEFT JOIN jetmon_site_check_config c ON c.blog_id = s.blog_id JOIN jetmon_site_tenants st ON st.blog_id = s.blog_id AND st.tenant_id = ? WHERE s.blog_id > ? ORDER BY s.blog_id ASC LIMIT ?`

const sitesListWithCLIMetadataSQL = ` SELECT s.blog_id, s.blog_id AS public_id, s.monitor_url, s.monitor_active, s.bucket_no, s.check_interval, s.site_status, s.last_checked_at, s.last_status_change, s.ssl_expiry_date, s.check_keyword, s.forbidden_keyword, s.forbidden_keywords, s.redirect_policy, c.request_method, c.detection_profile, s.maintenance_start, s.maintenance_end, s.alert_cooldown_minutes, s.custom_headers FROM jetpack_monitor_sites s LEFT JOIN jetmon_site_check_config c ON c.blog_id = s.blog_id WHERE s.blog_id > ? ORDER BY s.blog_id ASC LIMIT ?`

const singleSiteSQL = ` SELECT s.blog_id, s.blog_id AS public_id, s.monitor_url, s.monitor_active, s.bucket_no, s.check_interval, s.site_status, s.last_checked_at, s.last_status_change, s.ssl_expiry_date, s.check_keyword, s.forbidden_keyword, s.forbidden_keywords, s.redirect_policy, c.request_method, c.detection_profile, s.maintenance_start, s.maintenance_end, s.alert_cooldown_minutes FROM jetpack_monitor_sites s LEFT JOIN jetmon_site_check_config c ON c.blog_id = s.blog_id WHERE s.blog_id = ?`

const singleSiteWithCLIMetadataSQL = ` SELECT s.blog_id, s.blog_id AS public_id, s.monitor_url, s.monitor_active, s.bucket_no, s.check_interval, s.site_status, s.last_checked_at, s.last_status_change, s.ssl_expiry_date, s.check_keyword, s.forbidden_keyword, s.forbidden_keywords, s.redirect_policy, c.request_method, c.detection_profile, s.maintenance_start, s.maintenance_end, s.alert_cooldown_minutes, s.custom_headers FROM jetpack_monitor_sites s LEFT JOIN jetmon_site_check_config c ON c.blog_id = s.blog_id WHERE s.blog_id = ?`
...
func TestListSitesFiltersByMonitorActive(t *testing.T) {
	...
	expected := ` SELECT s.blog_id, s.blog_id AS public_id, s.monitor_url, s.monitor_active, s.bucket_no, s.check_interval, s.site_status, s.last_checked_at, s.last_status_change, s.ssl_expiry_date, s.check_keyword, s.forbidden_keyword, s.forbidden_keywords, s.redirect_policy, c.request_method, c.detection_profile, s.maintenance_start, s.maintenance_end, s.alert_cooldown_minutes FROM jetpack_monitor_sites s LEFT JOIN jetmon_site_check_config c ON c.blog_id = s.blog_id WHERE s.blog_id > ? AND s.monitor_active = 1 ORDER BY s.blog_id ASC LIMIT ?`
