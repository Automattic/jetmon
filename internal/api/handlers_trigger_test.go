...
const readSiteForCheckSQL = ` SELECT s.jetpack_monitor_site_id, s.blog_id, s.monitor_url, COALESCE(ec.timeout_seconds, c.timeout_seconds), COALESCE(ec.check_keyword, c.check_keyword), COALESCE(ec.forbidden_keyword, c.forbidden_keyword), COALESCE(ec.forbidden_keywords, c.forbidden_keywords), COALESCE(ec.custom_headers, c.custom_headers), COALESCE(ec.redirect_policy, c.redirect_policy), COALESCE(ec.request_method, c.request_method), COALESCE(ec.detection_profile, c.detection_profile), s.site_status FROM jetpack_monitor_sites s LEFT JOIN jetmon_endpoint_check_config ec ON ec.source_site_id = s.jetpack_monitor_site_id LEFT JOIN jetmon_site_check_config c ON c.blog_id = s.blog_id WHERE s.jetpack_monitor_site_id = ?`

var columnsSiteForCheck = []string{"jetpack_monitor_site_id", "blog_id", "monitor_url", "timeout_seconds", "check_keyword", "forbidden_keyword", "forbidden_keywords", "custom_headers", "redirect_policy", "request_method", "detection_profile", "site_status"}

const activeEndpointEventsSQL = `SELECT id FROM jetmon_events WHERE blog_id = ? AND endpoint_id = ? AND ended_at IS NULL`
const activeTriggerEventsSQL = `SELECT id FROM jetmon_events WHERE blog_id = ? AND (endpoint_id = ? OR endpoint_id IS NULL) AND ended_at IS NULL`
...
func TestTriggerNowSuccessNoActiveEvents(t *testing.T) {
	...
	mock.ExpectQuery(readSiteForCheckSQL).WithArgs(int64(42)).
		WillReturnRows(sqlmock.NewRows(columnsSiteForCheck).
			AddRow(int64(42), int64(42), target.URL, nil, nil, nil, nil, nil, "follow", nil, nil, 1))
	mock.ExpectQuery(activeTriggerEventsSQL).
...
func TestTriggerNowForbiddenKeywordFailsCheck(t *testing.T) {
	...
	mock.ExpectQuery(readSiteForCheckSQL).WithArgs(int64(42)).
		WillReturnRows(sqlmock.NewRows(columnsSiteForCheck).
			AddRow(int64(42), int64(42), target.URL, nil, nil, "FORBIDDEN", nil, nil, "follow", nil, nil, 1))
...
func TestTriggerNowSuccessClosesActiveEvent(t *testing.T) {
	...
	mock.ExpectQuery(readSiteForCheckSQL).WithArgs(int64(42)).
		WillReturnRows(sqlmock.NewRows(columnsSiteForCheck).
			AddRow(int64(42), int64(42), target.URL, nil, nil, nil, nil, nil, "follow", nil, nil, 2))
	mock.ExpectQuery(activeTriggerEventsSQL).
		WithArgs(int64(42), int64(42)).
