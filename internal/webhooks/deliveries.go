...
// rows with SELECT ... FOR UPDATE SKIP LOCKED inside a transaction so
// active-active delivery workers cannot claim the same row and do not wait
// behind rows another worker is already claiming. Each claimed row then gets an
