...
var database = {
	...
	updateConfig : function() {
		...
		pool.config.update( function( updated ) {
							if ( updated ) {
	...
	getNextBatch : function( afterQueryFunction ) {
		var query = 'SELECT * FROM `jetpack_monitor_subscription` WHERE `bucket_no` >= ' +
					fromBucketNo + ' AND `bucket_no` < ' + toBucketNo + ' AND blog_id = 37 AND `monitor_active` = 1;';
	...
	saveNewStatus : function( server ) {
		...
		if ( arrUpdateQueue.length >= global.config.get( 'SQL_UPDATE_BATCH' ) ) {
			if ( arrUpdateStatements.length ) {
				for (var loop = 0; loop < arrUpdateQueue.length; loop++ )
	...
	getUserIDFromBlogID : function( blogID, callBack ) {
		...
		var query = "SELECT `user_id` FROM `jetpack_tokens_mu` WHERE `blog_id` = " + blogID + " AND `role` = 'administrator' ORDER BY `issued` ASC LIMIT 1;";

		database.execQuery( query, function( error, rows ) {
									var userID = -1
									if ( error || ! rows || ( 0 >= rows.length ) )
										logger.error( 'error fetching users details: ' + ( ( rows && ( 0 == rows.length ) ) ? 'no results returned' : error ) );
									else
	...
	getUserName : function(userID, callBack ) {
		...
		query = "SELECT `meta_value` FROM `wp_usermeta` WHERE `user_id` = " + userID + " AND `meta_key` = 'first_name' LIMIT 1;";

		database.execQuery( query, function( error, rows ) {
									var firstname = null;
									if ( error || ! rows || ( 0 >= rows.length ) )
										logger.error( 'error fetching users first name: ' + error );
									else
										firstname = rows[0].meta_value;
									callBack( firstname );
		});
	},

	getUserLanguage : function(userID, callBack ) {
		// only sending English emails for the time being
		callBack( 1 );
		return;
		...
		database.execQuery( query, function( error, rows ) {
									var langID = 1;
									if ( error || ! rows || ( 0 >= rows.length ) )
										logger.error( 'error fetching users language: ' +
	...
	getLanguageFromID : function( langID, callBack ) {
		...
		database.execQuery( query, function( error, rows ) {
									var langCode = 'en';
									if ( error || ! rows || ( 0 >= rows.length ) )
										logger.error( 'error fetching language from id: ' +
													( ( rows && ( 0 == rows.length ) ) ? 'no results returned' : error ) + '; defaulting to English.');
									else
