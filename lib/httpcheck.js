...
// Last signing config pushed to the addon, to avoid re-parsing the key on
// every config reload (and to keep key swaps to actual rotations only).
var lastSigningConfig = null;
...
o_log4js.configure({
	...
	categories: {
		...
		slog: { appenders: ['slog'], level: 'trace' }
	}
});
...
var HttpChecker = {
	reloadConfig: function() {
		...
		// Sign outgoing site checks (RFC 9421, Web Bot Auth) when a signing key is configured.
		var disable_signing = function( reason ) {
			if ( null !== lastSigningConfig ) {
				_watcher.clear_signing();
				lastSigningConfig = null;
				logger.debug( process.pid + ': bot auth request signing disabled.' );
			}
			if ( reason )
				logger.error( process.pid + ': ' + reason + ', checks will be sent unsigned.' );
		};

		if ( ! config.get( 'BOT_AUTH_ENABLED' ) ) {
			disable_signing( null );
			return;
		}
		try {
			if ( ! config.get( 'BOT_AUTH_KEY_ID' ) || ! config.get( 'BOT_AUTH_DIRECTORY_URL' ) ) {
				disable_signing( 'BOT_AUTH_ENABLED but BOT_AUTH_KEY_ID or BOT_AUTH_DIRECTORY_URL is empty' );
				return;
			}
			var key_pem        = fs.readFileSync( config.get( 'BOT_AUTH_KEY_FILE' ), 'utf8' );
			var signing_config = key_pem + '|' + config.get( 'BOT_AUTH_KEY_ID' ) + '|' + config.get( 'BOT_AUTH_DIRECTORY_URL' );
			if ( signing_config === lastSigningConfig )
				return;

			if ( _watcher.configure_signing( key_pem, config.get( 'BOT_AUTH_KEY_ID' ), config.get( 'BOT_AUTH_DIRECTORY_URL' ) ) ) {
				lastSigningConfig = signing_config;
				logger.debug( process.pid + ': bot auth request signing enabled.' );
			} else {
				disable_signing( 'invalid bot auth signing key' );
			}
		}
		catch ( Exception ) {
			disable_signing( 'failed to load bot auth signing key: ' + Exception.toString() );
		}
	},
