		...
		if ( config.get( 'BOT_AUTH_ENABLED' ) ) {
			try {
				if ( ! config.get( 'BOT_AUTH_KEY_ID' ) || ! config.get( 'BOT_AUTH_DIRECTORY_URL' ) ) {
					logger.error( process.pid + ': BOT_AUTH_ENABLED but BOT_AUTH_KEY_ID or BOT_AUTH_DIRECTORY_URL is empty, checks will be sent unsigned.' );
					return;
				}
				...
				if ( _watcher.configure_signing( key_pem, config.get( 'BOT_AUTH_KEY_ID' ), config.get( 'BOT_AUTH_DIRECTORY_URL' ) ) ) {
					...
					logger.error( process.pid + ': invalid bot auth signing key, checks will be sent unsigned.' );
				}
			}
			catch ( Exception ) {
				logger.error( process.pid + ': failed to load bot auth signing key: ' + Exception.toString() );
			}
		}
	},
