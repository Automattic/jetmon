...
var HttpChecker = {
	...
	processResultsCallback: function( serverId, rtt, strDesc ) {
        ...
        server.checked++;
        server.site_status =  ~~( server.rtt > 0 && strDesc === "SITE OK");
        ...
        if ( server.site_status !=  server.oldStatus ) {
            ...
            if ( ! server.site_status && server.checked < config.NUM_OF_CHECKS ) {
                ...
                HttpChecker.statusChanged( server, strDesc );
            }
        }
    ...
    statusChanged: function( server, resp ) {
        process.send(  { msgtype: 'notify', server: server, resp: resp } );
    },
};
