...
function checkHostStatus( blog_id, status ) {
	for( var loop = 0; loop < queuedRetries.length; loop++ ) {
		if ( queuedRetries[ loop ].blog_id == blog_id ) {
			if ( HOST_OFFLINE == status ) {
...
function sslWorkerCallBack( msg ) {
	try {
		switch ( msg.msgtype ) {
			case 'host_status': {
				checkHostStatus( msg.payload.blog_id, msg.payload.status );
				break;
			}
			case 'host_status_array': {
				for( var loop = 0; loop < msg.payload.checks.length; loop++ ) {
					checkHostStatus( msg.payload.checks[ loop ].blog_id, msg.payload.checks[ loop ].status );
				}
				break;
			}
			default: {
