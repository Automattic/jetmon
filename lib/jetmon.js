...
function worker_msg_callback( msg ) {
    try {
		switch ( msg.msgtype ) {
            case "totals":
                if ( msg.server.processed == config.NUM_OF_CHECKS ) {
...
function update_sites_per_sec() {
	console.log( 'checks/sec = ' + checks_count + ' - Workers: ' + arrWorkers.length + '/' + numWorkers + ' working - sites processed: ' + sites_count + '/' + arrObjects.length );
	checks_count = 0;
