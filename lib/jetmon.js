    ...
    endOfRound = jetmonMysql.getNextBatch(
        function( rows ) {
            for ( var i = 0; i < rows.length; i++ ) {
                ...
                server.last_status_change = new Date(server.last_status_change).getTime();
    ...
    try {
		...
		switch ( msg.msgtype ) {
            ...
            case 'notify_status_change':
                ...
                if ( config.sendmails ) {
                    jetmonMailer.sendStatusChangeMail( msg.server );
                }
                break;
            case 'notify_still_down':
                if ( config.sendmails ) {
                	msg.server.site_status = 2;
	                jetmonMysql.saveNewStatus( msg.server );
                    jetmonMailer.sendStillDownMail( msg.server );
                }
                break;
