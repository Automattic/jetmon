...
cluster.on( 'exit', function( worker, code, signal ) {
    console.log(worker.process._events.error, code, signal);
...
function get_more_sites() {
    ...
    pool.getConnection(function(err, connection) {
        var query = 'SELECT * FROM jetpack_monitor_subscription WHERE bucket_no >= '
            + from_bucket_no + ' AND bucket_no < ' + to_bucket_no + ' AND monitor_status = 1'
        console.log(query);
    ...
    try {
		...
		switch ( msg.msgtype ) {
            ...
            case "i_need_some_work":
                ...
                var pointer_max = ( getWorker( msg.workerid ).id * bucket_size );
                ...
                //send some work
                getWorker( msg.workerid ).send( { id: msg.workerid, request: 'queue-add', payload: arrObjects.slice( pointer, pointer_max ) } );
...
function getWorker( pid ) {
    for (oworker in arrWorkers) {
        if ( arrWorkers[ oworker ].worker.process.pid == pid ) {
            return arrWorkers[ oworker ].worker;
        }
    }
    console.log( 'worker not found ' + pid );
    return null;
}
