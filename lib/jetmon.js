...
function maybeEndRound() {
	if ( inRound ) {
		logger.debug( 'maybeEndRound - check 1 --- arrObjects.length: ' + arrObjects.length + ', arrWorkers.length: ' + arrWorkers.length + ', freeWorkers.length: ' + freeWorkers.length );
	}
	if ( inRound && 0 === arrObjects.length && arrWorkers.length === freeWorkers.length ) {
		...
		logger.debug('maybeEndRound - check 2 --- round.complete.time (sec): ' + timeSinceStart/1000 + ', round.next.time (sec): ' + timeToNextLoop/1000);
