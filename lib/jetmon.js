...
o_log4js.configure( {
  appenders: [ {
	  ...
	  {
		  'type'      : 'file',
		  'filename'  : 'logs/processed-sites.log',
		  'maxLogSize': 104857600,
		  'backups'   : 100,
		  'category'  : 'processed-site',
		  'levels'    : 'DEBUG',
	  },
	]
});
o_log4js.PatternLayout = '%d{HH:mm:ss,SSS} m';
global.logger = o_log4js.getLogger( 'jetmon-main' );
var slogger   = o_log4js.getLogger( 'slog' );
var processed_site_log   = o_log4js.getLogger( 'processed-site' );

var infofunc = global.logger.info;
global.logger.info = (...msg) => {console.log(msg); infofunc(...msg)};
...
setInterval(() => {
	statsdClient.increment('jetmon.startups_test');
	//logger.debug('Trying to add stats');

	// statsdClient.set(['foo', 'bar'], 42, function(error, bytes){
	// 	//this only gets called once after all messages have been sent
	// 	if(error){
	// 		logger.debug('Oh noes! There was an error:', error);
	// 	} else {
	// 		logger.debug('Successfully sent', bytes, 'bytes');
	// 	}
	// });
}, 1000);

statsdClient.increment('jetmon.startups');
...
function spawnWorker() {
	var worker = child_proc.fork('./lib/httpcheck.js', [], { silent: true } );

	statsdClient.increment('jetmon.worker.spawn');

	logger.debug(`Spawned new worker with PID: ${worker.pid}`);

	worker.stdout.on('data', (data) => logger.debug('STDOUT: '+ data.toString().trim()));
	worker.stderr.on('data', (data) => logger.error('STDERR: '+ data.toString().trim()));
...
function workerMsgCallback( msg ) {
	statsdClient.increment('jetmon.worker.message');
	...
	try {
		switch ( msg.msgtype ) {
			...
			case 'notify_still_down':
				logger.debug('Worker message: '+msg.msgtype);
			...
			case 'send_work':
				/**
				 * If there are more workers than defined in the config,
				 * make the current worker exit.
				 *
				 * This is used to keep the number of workers in check in case something goes wrong.
				 */
				logger.debug('Worker message: '+msg.msgtype);
				...
				if ( arrWorkers.length > global.config.get( 'NUM_WORKERS' ) ) {
					...
					if ( null !== w ) {
						logger.debug('Too many workers, evaporating');
						w.send( {
							pid     : msg.worker_pid,
							request : 'evaporate',
							payload : 'pls :)'
				...
				if ( 0 == arrObjects.length ) {
					/**
					 * If there are no URLs in the queue to check, mark it as "free" and query more data from the db.
					 */
					logger.debug('arrObjects is empty');
					...
					/**
					 * There is data that needs processing, send DATASET_SIZE items to the queue of the worker.
					 */
					var w = getWorker( msg.worker_pid );
					if ( null !== w ) {
						logger.debug(`Sending data to worker: ${msg.worker_pid} - item count: `+ Math.min( arrObjects.length, global.config.get( 'DATASET_SIZE' )));
						w.send( {
							pid     : msg.worker_pid,
							request : 'queue-add',
			...
			case 'recheck':
				...
				break;
			case 'logger':
				logger.debug(`[${msg.worker_pid}] Processed URL: ${msg.data.monitor_url}`)
				processed_site_log.debug(msg.data.monitor_url);
			default:
				//logger.debug(msg);
...
function updateStats() {
	try {
		...
		/**
		 * Print out what the currently active workers are
		 */

		arrWorkers.forEach((worker) => {
			logger.debug('Active worker: '+worker.pid);
		})
	}
	catch  ( Exception ) {
