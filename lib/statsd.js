	...
	send: function( metric, value, type ) {
		...
		try {
			...
			socket.send( message, 8125, statsdHostname, (err) => {
				if ( err ) {
					console.log( 'Error when sending to statsd', {hostname: statsdHostname, message: message, err: err} );
				}
			});
		}
		catch ( Exception ) {
			console.log( 'Exception when sending to statsd', {hostname: statsdHostname, message: message, exception: Exception} );
		}
	}
}
